...
import advection_for_dns_solver
...
# assert (num_processes>5 and N>int(2**8)),('These parameters will occupy all of your computers memory..')
if num_processes > 1:
    ############
    # USER CHOICE
...
Np = 80000
ldx = L / N
particle_diffusion = 0.0005
# par_Pos_init = particle_IC(Np,L)
# particle_save_array=None
# split_coordinates = None
if rank == 0:
    par_Pos_init = particle_IC(Np, L, 'midNormal')
    particleData_old = par_Pos_init
...
###### Define advection-diffusion-paramters #######



local_field0, local_field = advection_for_dns_solver.advection_setup(N, L, rank, size, comm)


D = 0.0005
dx= dy= dz = 1 / N
dx2= dy2= dz2 = dx ** 2
save_field_step = 1000
rx = dt * D / (dx2)
ry = dt * D / (dy2)
rz = dt * D / (dz2)
r3 = rx + ry + rz
assert (
           r3) < 0.5, 'value of r should be smaller than 0.5, check parameters dt,dx**2 and D. Now the value of is r=' + str(
    rx + ry + rz)
...
def transform_Uc_zx(Uc_hat_z, Uc_hat_xr, P1):
    sz = Uc_hat_z.shape
    sx = Uc_hat_xr.shape
    Uc_hat_z[:, :, :-1] = rollaxis(Uc_hat_xr.reshape((P1, sz[0], sz[1], sx[2])), 0, 3).reshape(
        (sz[0], sz[1], sz[2] - 1))
    return Uc_hat_z
...
def computeRHS(dU, rk):
    ...
    # if rank==0:
    # print('Energy in forcing band: '+str(kinBand))
    return dU


def dynamicPostProcess(tstep, plot_step, u_reshaped):
    global kinBand
    if tstep % plot_step == 0:
        ...
        print('Energy in total band: ' + str(kinTotal))
...
def computeParticle(u_reshaped, particleData_old, tstep):
    '''
    if rank==0:
        print('before broadcasting u_reshaped',flush=True)
    u_reshaped = comm.bcast(u_reshaped, root=0)
    if rank==0:
        print('after broadcasting u_reshaped',flush=True)
    '''
    f = Interpolator(u_reshaped)
    # del(u_reshaped)
    particleData_new = trajectory(tstep, Tend, h, f, Euler, True, L, ldx, particleData_old)
    # del(f)
    return particleData_new


def saveParticleCoord_MPI(particleData_new, particle_save_array, tstep):
...
def saveParticleCoord(particleData_new, particle_save_array, tstep):
    if tstep%10==0:
        particle_save_array = concatenate((particle_save_array, array([particleData_new])), axis=0)
    if (tstep % 500 == 0):
        # TODO implement feature that makes new save array after some tstep, to reduce size of numpy file
        save('particleCoord.npy', particle_save_array)
        print('save at tstep: ' + str(tstep), flush=True)
    return particle_save_array
...
if __name__ == '__main__':
    ...
    if IC == 'isotropic2':
        ...
        for i in range(3):
            U[i] = ifftn_mpi(U_hat[i], U[i])
            # TODO add random contribution to this field
    if IC == 'TG':
    ...
    ###################################################################################################################
    ######## RK 2-3 routine for propagating the initial velocity field in time using a pseudo-spectral method #########
    ...
    while t < Tend - 1e-8:
        ...
        ###############################################################################################
        ########################## End of spectral method routine #####################################

        ################################################################################################################
        ###################### Computation of particle trajectory and Advection-Diffusion equation #####################


        u_gathered = comm.gather(U, root=0)
        if rank == 0:
            # TODO Make sure that rank==0 is not a problem for advection-diffusion parallellization!!
            u_reshaped = reshapeGathered(u_gathered, N, N1, N2, P1, P2, num_processes, 'concatenate')
            particleData_new = computeParticle(u_reshaped, particleData_old, tstep)
            particleData_old = particleData_new
            particle_save_array = saveParticleCoord(particleData_new, particle_save_array, tstep) # Save coordinate of particles
            dynamicPostProcess(tstep, plot_step, u_reshaped) #Do post processing, save arrays or make plots etc
            # if tstep%10==0:
            # plotScatter(fig_particle, ax_particle, tstep, particle_save_array, rgbaTuple, pointSize, L, pointcolor1, pointcolor2, Np)

        else:
            u_reshaped = None
            #kinBand=None
        ### Advection-diffusion routine here ####
        local_field,local_field0= advection_for_dns_solver.iterate(tstep, save_field_step, u_reshaped, local_field, local_field0,dx,dy,dz,dx2,dy2,dz2,D,dt, rank, size,
                                         comm)

        #########################################
        kinBand = comm.bcast(kinBand, root=0)  # Update energy in velocity field
        tstep += 1
        mpiPrintIteration(tstep) # Print out timestep
