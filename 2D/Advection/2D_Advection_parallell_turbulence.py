...
def iterate(field, local_field, local_field0, timesteps, image_interval,field_store_counter):
    ...
    for t in range(0, timesteps ):
        if (t == 0):
            u_init = np.load(str(path.parent)+'/vorticity_formulation/'+filenames_u[0])
            v_init = np.load(str(path.parent)+'/vorticity_formulation/'+filenames_v[0])
            shape = u_init.shape
            dtype = u_init.dtype
        ...
        if (t != 0):
            if (t%breaker==1):
                u = np.load(str(path.parent)+'/vorticity_formulation/'+filenames_u[indexnr])
                v = np.load(str(path.parent)+'/vorticity_formulation/'+filenames_v[indexnr])
                indexnr +=1
                shape = u[0].shape
                dtype = u[0].dtype
                comm.bcast(shape, 0)  # broadcast dimensions
        ...
        if (t %breaker == 0):
            ...
            if (rank == 0):
                print('hello')
                np.save('datafiles/concentration/field_' + str(round(t)), field)
                #field_store_counter += 1
